...
u_0= Expression('sin(x[0])+cos(x[1])', degree=2)
...
a_sys1=u*v*dx+D*dot(grad(u), grad(v))*dt/float(2)*dx
...
a_sys2=u*v*dx+dot(B, grad(u))*v*dt/float(4)*dx
L_sys2=u_n*v*dx+f*v*dt/float(4)*dx

...
class PeriodicBoundary(SubDomain):
    ...
    def inside(self, x, on_boundary):
        ...
        return bool((near(x[0], 0) or near(x[1], 0)) and 
                (not ((near(x[0], 0) and near(x[1], 2*DOLFIN_PI)) or 
                        (near(x[0], 2*DOLFIN_PI) and near(x[1], 0)))) and on_boundary)
...
f = Expression('b_1*exp(-t)*cos(x[0]) - b_2*exp(-t)*sin(x[1])', degree=2, t=0, b_1=1.0, b_2=1.0)
u_n = project(u_0, V)

a = u*v*dx + dt*dot(grad(u), grad(v))*dx + dt*grad(u)[0]*v*dx
L = (u_n + dt*f)*v*dx
